...
function promptMetricToCode(): string[] {
  return [
    ...
    'as reported by bpmn-js, NOT center coordinates.',
    '',
    'X-axis: snapLeft() already snaps left edges correctly. Do NOT change it.',
    '',
    'Y-axis: this is where the remaining gridSnap penalties come from.',
    'The Y coordinate for lane elements is set in buildElementLaneYMap() in lane-layout.ts:',
    '  cyMap: laneId → l.y + (l.height || DEFAULT_LANE_HEIGHT) / 2',
    'If lane.height is not a multiple of 20, then center Y = l.y + odd_number → off-grid.',
    '',
    'CONCRETE FIX (lane scenarios S06/S07/S12):',
    'In buildElementLaneYMap() in src/rebuild/lane-layout.ts, snap the lane center Y:',
    '  .map((l) => [l.id, Math.round((l.y + (l.height || DEFAULT_LANE_HEIGHT) / 2) / 10) * 10])',
    '',
    'WARNING: Do NOT fix this in resizePoolAndLanes() — that function runs AFTER elements are',
    'already positioned. Changes there have ZERO effect on gridSnapAvg.',
    'The fix MUST go in buildElementLaneYMap() which runs BEFORE computePositions().',
    '',
    'After fixing lane Y, also check non-lane scenarios: branchSpacing increments in',
    'computePositions() (src/rebuild/positioning.ts) may produce off-grid Y values.',
    '',
    '### bendCount',
